...
BPTreeFixed::BPTreeFixed(const std::string& filename) : filename(filename), root_pos(-1), node_count(0) {
    ...
    if (!exists) {
        ...
        // Write header
        int header[2] = {-1, 0};  // root_pos, node_count
...
int BPTreeFixed::allocate_node() {
    int pos = sizeof(int) * 2 + node_count * sizeof(BPTNodeFixed);
    node_count++;
    return pos;
}

void BPTreeFixed::write_node(int pos, const BPTNodeFixed& node) {
    file.seekp(pos);
    file.write(reinterpret_cast<const char*>(&node), sizeof(BPTNodeFixed));
}

BPTNodeFixed BPTreeFixed::read_node(int pos) {
    BPTNodeFixed node;
    file.seekg(pos);
    file.read(reinterpret_cast<char*>(&node), sizeof(BPTNodeFixed));
    return node;
}

int BPTreeFixed::find_key_index(const BPTNodeFixed& node, const std::string& key) {
    int idx = 0;
    while (idx < node.key_count && std::string(node.keys[idx]) < key) {
        idx++;
    }
    return idx;
}

void BPTreeFixed::insert(const std::string& key, int value) {
    if (root_pos == -1) {
    ...
    BPTNodeFixed current = read_node(current_pos);
...
void BPTreeFixed::split_leaf(int leaf_pos, BPTNodeFixed& leaf) {
    BPTNodeFixed new_leaf;
...
void BPTreeFixed::split_internal(int int_pos, BPTNodeFixed& internal) {
    BPTNodeFixed new_internal;
    ...
    for (int i = 0; i <= new_internal.key_count; i++) {
        if (new_internal.children[i] != -1) {
            BPTNodeFixed child = read_node(new_internal.children[i]);
...
void BPTreeFixed::insert_in_parent(int left_pos, const std::string& key, int right_pos) {
    if (left_pos == root_pos) {
        ...
        BPTNodeFixed left = read_node(left_pos);
        left.parent = new_root_pos;
        write_node(left_pos, left);

        BPTNodeFixed right = read_node(right_pos);
    ...
    BPTNodeFixed parent = read_node(parent_pos);
...
std::vector<int> BPTreeFixed::find(const std::string& key) {
    ...
    while (current_pos != -1) {
        ...
        if (found) {
            // Check next leaf
            if (current.next != -1) {
                BPTNodeFixed next = read_node(current.next);
...
void BPTreeFixed::remove(const std::string& key, int value) {
    if (root_pos == -1) {
        return;
    }
    ...
    while (leaf_pos != -1) {
        ...
        for (int i = 0; i < current.key_count; i++) {
            if (std::string(current.keys[i]) == key && current.values[i] == value) {
                // Remove from leaf
